...
const DctIndex = function (fieldList, opts) {
  ...
  this.rowIdFieldName = this.mapper.idAttribute || opts.idAttribute || 'id' // For Tests..  this.idxFieldName
...
DctIndex.prototype.set = function (keyList, value) {
  ...
  if (this.bPrimaryKeyIdx) {
    if (utils.isUndefined(idxValue)) {
      let idxFound = this.arrUndefinedKeyRows.indexOf(value)
      if (idxFound == -1) // prevent duplicates
    ...
    if (utils.isUndefined(idxValue)) {
      let idxFound = this.arrUndefinedIndexKeyRows.indexOf(value)
      if (idxFound == -1) // prevent duplicates
...
DctIndex.prototype.removeRecord = function (data) {
  ...
  if (!(rowIdValue in this.dctIndexValueByRowId)) {
    ...
    let foundIdx = this.arrUndefinedIndexKeyRows.indexOf(data)
    if (foundIdx >= 0) {
      let itemsRemoved = this.arrUndefinedIndexKeyRows.splice(foundIdx, 1)
  ...
  this.dctRows[storedIdxValue] = arrRows.filter(function (val, idx) {
    let elementRowId = val[idAttribute]
