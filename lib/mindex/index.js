  ...
  getAll (opts) {
    ...
    if (opts.order === 'desc') {
      for (let i = values.length - 1; i >= 0; i--) {
        ...
        if (value.isIndex) {
          results.push.apply(results, value.getAll(opts))
        } else {
    ...
    if (leftKeys.length === 0) {
      ...
      for (let i = pos.index; i < this.keys.length; i += 1) {
        ...
        if (this.values[i].isIndex) {
          results.push.apply(results, this.values[i].getAll())
        } else {
          results.push.apply(results,this.values[i])
        }
      ...
      for (let i = pos.index; i < this.keys.length; i += 1) {
        ...
        if (this.values[i].isIndex) {
          if (currKey === leftKey) {
            results.push.apply(results,this.values[i]._between(utils.copy(leftKeys), rightKeys.map(function () { return undefined }), opts))
          } else if (currKey === rightKey) {
            results.push.apply(results,this.values[i]._between(leftKeys.map(function () { return undefined }), utils.copy(rightKeys), opts))
          } else {
            results.push.apply(results,this.values[i].getAll())
          }
        } else {
